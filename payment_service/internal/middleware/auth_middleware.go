...
func AuthMiddleware(logger *zap.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			logger.Warn("Invalid token payload", zap.Error(err))
		...
		if err := json.Unmarshal(payloadJSON, &payloadMap); err != nil {
			logger.Warn("Invalid token payload format", zap.Error(err))
		...
		if !ok {
			logger.Warn("User ID has invalid format")
		...
		logger.Debug("User authenticated", zap.Int("user_sso_id", userSSOID))
		c.Next()
	}
}
