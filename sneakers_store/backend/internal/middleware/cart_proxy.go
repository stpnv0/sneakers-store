...
// CartProxyMiddleware создает middleware для проксирования запросов к микросервису корзины
func CartProxyMiddleware(cartServiceURL string) gin.HandlerFunc {
	fmt.Printf("DEBUG: CartProxyMiddleware initialized with URL: %s\n", cartServiceURL)
	...
	return func(c *gin.Context) {
		...
		if strings.HasSuffix(cartServiceURL, "/api/v1") {
			// Если cartServiceURL уже содержит /api/v1, просто добавляем путь
			relPath := c.Request.URL.Path[7:] // Убираем начальные /api/v1
			targetURL = fmt.Sprintf("%s%s", cartServiceURL, relPath)
		} else {
			// Иначе используем полный путь
			targetURL = fmt.Sprintf("%s%s", cartServiceURL, c.Request.URL.Path)
		}
		...
		if err != nil {
			c.JSON(http.StatusBadGateway, gin.H{"error": "Failed to reach cart microservice"})
			c.Abort()
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to read response from cart microservice"})
			c.Abort()
			return
		}
